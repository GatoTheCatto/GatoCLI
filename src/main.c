...
static void print_usage(const char *prog) {
	...
	puts("  greet [name]                 Print a friendly greeting (default: "
		 "there)");
	puts(
		"  repeat [count]               Print a numbered notification multiple "
		"times");
...
static void repeat_notification(int count) {
	for (int i = 1; i <= count; ++i) {
		// show each notification with its number so the caller can tell how
		// many happened.
...
int main(int argc, char *argv[]) {
	...
	if (strcmp(command, "add") == 0) {
		...
		if (argc >= 4) {
			...
			fprintf(stderr,
					"Please put at least 2 arguments for this option\n");
			return 1;
		}
	...
	if (strcmp(command, "repeat") == 0) {
		...
		if (argc >= 3) {
			...
			fprintf(stderr,
					"Please put at least 1 argument for this option.\n");
			return 1;
		}
	...
	if (strcmp(command, "hello") == 0) {
		...
		// implemented yet! This will repeat %d times, despite your input of
		// %d.", times, parse_positive_int(argv[2]));
		return 0;
	}
	...
	if (strcmp(command, "count") == 0) {
		...
		printf("There are %d occurrences of '%c'",
			   count_occurrences(argv[2], *argv[3]),
